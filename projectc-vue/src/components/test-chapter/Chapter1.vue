<template>
    <div class="test-box">
        ...
        <div class="type-title">四、程序填空题（共36分，3题，每题12分,每空4分）</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getChapterData(){
                axios({
                    method: 'post',
                    url: '/chapter',
                    data: this.$qs.stringify({chapterIndex:1})
                }).then(res=>{
                    console.log(res);
        ...
        mounted() {
           ...
           /* this.timer.seconds--;       //为了让seconds变成0
            this.formatTimer();         //执行第一次formatTimer方法
            this.formatTimer();         //第二次*/
            //刷新或者关闭页面的时候进行提示
