    ...
    <div class="test-box">
        <el-col span="6">
            <el-container style="margin-left:1350px;position: fixed;">
                <el-header>选择题库练习—25题</el-header>
        ...
        <div class="type-title">选择题库-25题（共100分，25题，每题4分）</div>
        <ChoicesItem style="margin-left: 140px"   :class="`choiceJump${index+1}`" v-for="(choice,index) in choiceArray" :index="index" :item="choice"/>
        <el-button @click="changeVisibleToTrue" type="primary" plain style="width: 160px;height: 50px;margin-left: 625px;text-align: center;">提交</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeVisibleToTrue(){
                if (this.choiceValues.length!==25){
            ...
            getChapterData(){
                axios({
                    method: 'post',
                    url: '/bank_choice',
                    data: this.$qs.stringify({choiceN:25})
                }).then(res=>{
                    console.log(res);
